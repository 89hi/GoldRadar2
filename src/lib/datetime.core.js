...
(function ($) {
    ...
    function Scroller(elem, settings) {
        ...
        function read() {
            if(that.settings.$outel){
                var outval = that.settings.$outel.text();
                that.temp = ((input && that.val !== null && that.val != outval) || that.values === null) ? s.parseValue(outval || '', that) : that.values.slice(0);
            }else{
