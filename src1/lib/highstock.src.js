...
Pointer.prototype = {
  ...
  normalize: function (e, chartPosition) {
    ...
    if (ePos.pageX === UNDEFINED) { // IE < 9. #886.
      ...
      // console.log('ePos.pageX :'+ePos.pageX);
      if(e.swccounter){
        chartX = ePos.pageX + e.swccounter - chartPosition.left;
        // console.log('pageX :'+ePos.pageX+'|swccounter :'+e.swccounter);
   ...
   */
  changePointActions: function (leftflg,e) {
    var pointer = this,
      chart = pointer.chart,
      series = chart.series,
    ...
    if(!this.prevKDPoint){
      ...
      if(this.previndex >= series[0].points.length){
        this.previndex = series[0].points.length-1;
      }
    }

    function searchPointBySerie(serie,timestamp){
      var points = serie.points
      for(var i = 0; i < points.length; i++){
        if(points[i].x == timestamp){
          return points[i]
        }
      }
      return null
    ...
    // debugger
    (this.prevKDPoint && this.prevKDPoint.series) || (this.prevKDPoint = series[0].points[this.previndex]);
    this.prevKDPoint = searchPointBySerie(series[0],this.prevKDPoint.x);
    // (!this.prevKDPoint)&&(this.prevKDPoint = series[3].points[this.previndex],this.prevKDPoint = searchPointBySerie(series[3],this.prevKDPoint.x));
    kdpoint = this.prevKDPoint.series.points[this.prevKDPoint.index+action];

    var tmpPoint,_this = this,searchPoint;
    // console.log('series length:',series.length);
    each(series, function (s) {
      tmpPoint = null;
      // if(_this.previndex < s.points.length && pick(s.options.enableMouseTracking, true)){
      noSharedTooltip = s.noSharedTooltip && shared;
      directTouch = !shared && s.directTouch;
      
      if (s.visible && !noSharedTooltip && !directTouch && pick(s.options.enableMouseTracking, true)) { // #3821
        tmpPoint = searchPointBySerie(s,kdpoint.x)
        // tmpPoint = s.points[_this.previndex+action];
        if(tmpPoint){
          // if(tmpPoint.baseflg){
          //   // console.log('s.points.length:',s.points.length);
          //   for(var i = 0; i < s.points.length; i++){
          //     searchPoint = s.points[i];
          //     // console.log('searchPoint:',searchPoint,searchPoint.x == kdpoint.x);
          //     if(searchPoint.x == kdpoint.x){
          //       tmpPoint = searchPoint
          //     }
          //   }
          // }
          kdpoints.push(tmpPoint);  
        }
      }else{
        // console.log(s.visible,!noSharedTooltip , !directTouch , pick(s.options.enableMouseTracking, true));
      }
    });
    if(!kdpoint){
      return;
    }
    e.clientX = e.chartX = e.x = kdpoint.clientX;
  ...
  getAllItems: function () {
    ...
    each(this.chart.series, function (series) {
      ...
      if(isArray(tmpitems)){
        ...
        if(!tmpitems.hidelegend){
          allItems.push(tmpitems);
        }
      }
...
Scroller.prototype = {
  ...
  render: function (min, max, pxMin, pxMax) {
    // if(!this.scrollbarOptions.showScroller){
    //   return;
    // }
    var scroller = this,
      chart = scroller.chart,
...
RangeSelector.prototype = {
  ...
  render: function (min, max) {
    ...
    if (!rendered) {
      ...
      if(!rangeSelector.options.hide){
        rangeSelector.zoomText.add(buttonGroup);
      }
